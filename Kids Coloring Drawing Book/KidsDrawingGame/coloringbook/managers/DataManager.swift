...
//  coloringbook
//
//  Created by AMAN JAIN on 11/6/16.
...
class DataManager {
    ...
    func getThumbImageName(categoryIndex: Int, imageIndex: Int) -> String{
        ...
        let imageName = jsonData["categories"][categoryIndex]["images"][imageIndex]["big"].stringValue
        let name = categoryName + "_big_" + imageName + ".png"
...
//        return UIImage(named: jsonData["categories"][selectedCategoryIndex]["images"][selectedImageIndex]["thumb"].stringValue)
    ...
    func deleteAllImages() {
        ...
        do {
            let fileURLs = try fileManager.contentsOfDirectory(at: documentsDirectoryPath, includingPropertiesForKeys: nil, options: .skipsHiddenFiles)
